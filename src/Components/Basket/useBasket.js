...
// import { getQuery } from "../useProductView";
...
const useBasket = (token, setIsNewOrdersAdded) => {
  ...
  // Function to save the basket items to localStorage
  const saveBasketToLocalStorage = (cartItems) => {
    localStorage.setItem("cartItems", JSON.stringify(cartItems));
  };

  // Function to get the basket items from localStorage
  ...
  useEffect(() => {
    ...
    const storedCartItems = getBasketFromLocalStorage();
    setBasket(storedCartItems);
  }, []);

  useEffect(() => {
    // Update the localStorage whenever the basket changes
